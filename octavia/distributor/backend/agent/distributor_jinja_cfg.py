...
TEMPLATES_DIR = (os.path.dirname(os.path.realpath(__file__)) +
                 constants.DISTRIBUTOR_AGENT_API_TEMPLATES + '/')


class DistributorJinjaTemplater(object):
    ...
    def __init__(self):
        ...
        self.agent_template = jinja_env.get_template(
            constants.DISTRIBUTOR_AGENT_CONF_TEMPLATE)

    def build_agent_config(self, distributor_id):
        return self.agent_template.render(
            {'agent_server_ca': CONF.amphora_agent.agent_server_ca,
             ...
             'bind_port': CONF.distributor.bind_port,
